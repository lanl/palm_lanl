...
! Former revisions:
! -----------------
! $Id: vertical_nesting_mod.f90 3083 2018-06-19 14:03:12Z gronemeier $
! Error messages revised
! 
! 3065 2018-06-12 07:03:02Z Giersch
! dz was replaced by dz(1), error messages related to vertical grid stretching 
! have been added
! 
...
! 
! 2712 2017-12-20 17:32:50Z kanani
! Formatting and clean-up (SadiqHuq)
! 
...
!------------------------------------------------------------------------------!
 MODULE vertical_nesting_mod
    ...
    LOGICAL                                   ::  vnested = .FALSE.            !> set to true when
                                                                               !> mrun is called with -N option
    ...
    INTEGER(iwp)                              :: cg_nprocs        !> no. of PE in CG. Set by mrun -N
    INTEGER(iwp)                              :: fg_nprocs        !> no. of PE in FG. Set by mrun -N
    ...
    INTEGER(iwp)                              :: n_cell_f         !> total no. of FG grid points in a PE
...
!-- Initialize old time levels
    ...
    CONTAINS
       ...
       SUBROUTINE interpolate_to_fine_w( tag )
       ...
       SUBROUTINE interpolate_to_fine_u( tag )
       ...
       SUBROUTINE interpolate_to_fine_v( tag )
       ...
       SUBROUTINE interpolate_to_fine_s( tag )
       ...
       SUBROUTINE interpolate_to_fine_kh( tag )
           ...
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: wprs
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: ptprs
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: eprs
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: kmprs
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: khprs
           REAL(wp), DIMENSION(:,:), ALLOCATABLE   :: tspr 
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: uprf
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: vprf
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: wprf
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: ptprf
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: eprf
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: kmprf
           REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: khprf
       ...
       SUBROUTINE interpolate_to_fine_km( tag )
       ...
       SUBROUTINE interpolate_to_fine_flux( tag )
...
!-- Limit anterpolation level to nzt - z nesting ratio (a pseudo-buffer layer)
    ...
    CONTAINS
       SUBROUTINE anterpolate_to_crse_u( tag )
       ...
       SUBROUTINE anterpolate_to_crse_v( tag )
       ...
       SUBROUTINE anterpolate_to_crse_w( tag )
       ...
       SUBROUTINE anterpolate_to_crse_s( tag )
    ...
    SUBROUTINE vnest_anterpolate_e
        ...
        ELSEIF ( coupling_mode == 'vnested_fine' )  THEN
            ...
            CALL anterpolate_to_crse_e ( 104 )
    ...
    CONTAINS 
       ...
       SUBROUTINE anterpolate_to_crse_e( tag )
...
! ceiling(dxc/dxf) 
       ...
       USE control_parameters,                                                    &
           ONLY:  coupling_mode, coupling_mode_remote, coupling_topology, dz
...
#if defined( __parallel )
         USE control_parameters,                                                    &
             ONLY:  coupling_mode, dt_3d, old_dt
...
!-- Identical timestep for coarse and fine grids
          dt_3d = MIN( dtc, dtf )
          !> @fixme setting old_dt might be obsolete at this point
          !>   Due to changes in timestep routine, setting of old_dt might be
          !>   not necessary any more at this point. However, could not be
          !>   tested so far.
          !>   2018-05-18, gronemeier
          old_dt = dt_3d
#endif
