...
! Former revisions:
! -----------------! 
! $Id: pmc_particle_interface.f90 3049 2018-05-29 13:52:36Z Giersch $
! Error messages revised
! 
! 2967 2018-04-13 11:22:08Z Giersch
! bugfix: missing parallel cpp-directives added
! 
! 2884 2018-03-14 08:33:20Z scharf
! B: corrected KIND of variable "parsize" for some MPI calls
! 
...
!------------------------------------------------------------------------------!
   ...
   USE pmc_interface,                                                          &
       ONLY: cpl_id, get_number_of_childs, nr_part,  part_adr, nested_run,     &
             get_childid, get_child_edges, nr_partc,  part_adrc,               &
             coarse_bound, coord_x, coord_y, cg,   get_child_gridspacing,      &
...
!------------------------------------------------------------------------------!
 SUBROUTINE pmcp_g_init
    ...
    nr_childs = get_number_of_childs()
!
...
!--    compute number of fine grid cells in coarse grid (one direction)
       xx = (cg%dx + eps) / dx ! +eps to avoid rounding error
       yy = (cg%dy + eps) / dy
       nr_fine_in_coarse = MAX( INT(xx), INT(yy) )

       IF ( MOD( coord_x(0), cg%dx ) /= 0.0 .OR. MOD( coord_y(0), cg%dy ) /= 0.0 ) THEN
...
!------------------------------------------------------------------------------!
 SUBROUTINE pmcp_p_fill_particle_win
    ...
    REAL(wp) ::  xo         !< origin of particle
    REAL(wp) ::  y          !< particle position
    REAL(wp) ::  yo         !< origin of particle
...
!--    Therfore p_copy_particle_to_org_grid is done for one child process per call

       DO ip = 1, pmc_s_get_child_npes( child_id )
          nr_part  = 0
          part_adr = 0

          CALL pmc_s_getdata_from_buffer( child_id, particle_transfer = .TRUE., child_process_nr = ip )

          CALL p_copy_particle_to_org_grid( m, child_id )
       ENDDO
...
!------------------------------------------------------------------------------!
 SUBROUTINE c_copy_particle_to_coarse_grid
    ...
    REAL(iwp) ::  x       !< x coordinate
    REAL(iwp) ::  xo      !< x origin
    REAL(iwp) ::  x_left  !< absolute left boundary
    REAL(iwp) ::  x_right !< absolute right boundary
    REAL(iwp) ::  y       !< left boundary in coarse(parent) index space
    REAL(iwp) ::  yo      !< y origin
    REAL(iwp) ::  y_north !< absolute north boundary
    REAL(iwp) ::  y_south !< absoulte south boundary
    REAL(iwp) ::  z       !< z coordinate
             ...
             DO  n = 1, number_of_particles
                IF ( particles(n)%particle_mask )  THEN
                   ...
                   IF ( boundary_particle ) THEN                     
                      ic = x / cg%dx                     !TODO anpassen auf Mehrfachnesting
                      jc = y / cg%dy
...
!------------------------------------------------------------------------------!
 SUBROUTINE p_copy_particle_to_org_grid( m, child_id )
