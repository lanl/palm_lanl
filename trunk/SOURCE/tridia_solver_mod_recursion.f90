...
call cpu_log(log_point_s(7), 'tridiag', 'start')
!
             ...
             DO  j = nys_z, nyn_z
                DO  i = nxl_z, nxr_z
                    ...
                    do kk = 1,k
                       prod = 1.0_wp
                       do l = 1,k-kk
                            prod = prod * tri(i,j,kk,2)
                       enddo
                       sumV = sumV + prod*ar(i,j,kk)*(-1.0_wp)**(k-kk)
                    enddo
                      ar1(i,j,k) = ar(i,j,k+1) - sumV 
...
!--       acceleration of horizontally averaged vertical velocity is zero.
          IF ( ibc_p_b == 1  .AND.  ibc_p_t == 1 )  THEN
             ...
             ENDIF
          ENDIF
call cpu_log(log_point_s(7), 'tridiag', 'stop')
