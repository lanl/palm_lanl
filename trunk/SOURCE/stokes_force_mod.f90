...
!------------------------------------------------------------------------------!
    SUBROUTINE stokes_pressure_head
       ...
       USE arrays_3d,                                                          &
           ONLY:  p, u, v, u_stk, v_stk, rho_ref_zu
...
! TODO: Check if the stokes pressure head is correctly scaled by rho_ref_zu,
!       as rho_ref_zu is used in pres.f90 to calculate p
!       <20180926, Qing Li> !
!
...
!--             Predetermine flag to mask topography
                flag = MERGE( 1.0_wp, 0.0_wp,                                  &
                           BTEST( wall_flags_0(k,j,i), 0 ) )
                p(k,j,i) = p(k,j,i) - 0.5_wp * rho_ref_zu(k) *                    &
